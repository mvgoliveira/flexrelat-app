...
export const AiChat = (): ReactElement => {
    ...
    const handleSendMessage = async () => {
        if (chatMessage.trim().length > 0 && modelData) {
            try {
                ...
                const aiResponse = await sendMessage(modelData.id, "models", chatMessage.trim());
    ...
    const handleClearMessages = async () => {
        if (modelData) {
            try {
                await clearMessagesByRelatedId(modelData.id, "models");
