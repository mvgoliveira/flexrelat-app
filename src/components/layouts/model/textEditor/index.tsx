...
const TextEditor = ({
        ...
        () =>
            _.debounce(async (newContent: string) => {
                try {
                    ...
                    await handleChangeModelContent(newContent);
                    updateSaveStatus("success");
                } catch (error) {
                    updateSaveStatus("error");
                }
            }, 1000),
        [handleChangeModelContent, updateSaveStatus]
    );
    ...
    const saveNow = useCallback(async () => {
        ...
        try {
            updateSaveStatus("pending");
            await handleChangeModelContent(currentEditor.getHTML());
            updateSaveStatus("success");
        } catch (error) {
            updateSaveStatus("error");
        }
    ...
    useLayoutEffect(() => {
        if (!alreadyLoaded.current && modelData && currentEditor) {
            currentEditor
                .chain()
                .setContent(modelData.content, {
